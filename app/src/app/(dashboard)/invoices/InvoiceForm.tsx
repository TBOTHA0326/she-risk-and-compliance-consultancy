...
import type { Invoice, InvoiceLineItem, Company } from '@/types/database'
...
export function InvoiceForm({ invoice, companies }: InvoiceFormProps) {
  ...
  const [form, setForm] = useState({
    ...
    company_id: invoice?.company_id ?? defaultCompany,
  ...
  const checkNumberUnique = useCallback(async (num: string) => {
    if (!num) return
    const { data } = await supabase.rpc('check_invoice_number_unique', {
      p_invoice_number: num,
      p_exclude_id: invoice?.id ?? undefined,
    })
    if (data === false) setNumberError('Invoice number already exists')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await supabase.from('activity_log').insert({
      ...
      entity_type: 'invoice',
      entity_id: invoiceId!,
      entity_label: form.invoice_number,
    })
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        action={<Link href={isEdit ? `/invoices/${invoice!.id}` : '/invoices'}><Button variant="secondary">Cancel</Button></Link>}
      />
